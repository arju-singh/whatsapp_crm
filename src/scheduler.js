...
function scheduleFollowups() {
  ...
  const recentWa = db.prepare(`
    SELECT id, vendor_id, sent_at FROM messages
    WHERE direction = 'out' AND status IN ('sent','delivered','read')
      AND sent_at IS NOT NULL AND sent_at >= ?
  `).all(since);
  const recentEmail = db.prepare(`
    SELECT id, vendor_id, sent_at FROM emails
    WHERE direction = 'out' AND status IN ('sent','delivered','opened')
      AND sent_at IS NOT NULL AND sent_at >= ?
  `).all(since);

  const insert = db.prepare(`
    INSERT INTO followups (rule_id, vendor_id, parent_message_id, attempt, status, scheduled_at)
    VALUES (?, ?, ?, 1, 'pending', ?)
  `);
  const exists = db.prepare(`
    SELECT 1 FROM followups WHERE rule_id = ? AND vendor_id = ? AND parent_message_id = ?
  `);

  for (const rule of rules) {
    // Trigger source: a rule schedules from the SAME channel's prior send (WA rule looks at WA sends, email rule looks at email sends).
    const source = rule.channel === 'email' ? recentEmail : recentWa;
    for (const m of source) {
      if (exists.get(rule.id, m.vendor_id, m.id)) continue;
      const scheduledAt = m.sent_at + rule.delay_hours * HOUR_MS;
      insert.run(rule.id, m.vendor_id, m.id, scheduledAt);
    }
  }
}
...
function fireDueFollowups() {
  ...
  const due = db.prepare(`
    ...
    WHERE f.status = 'pending' AND f.scheduled_at <= ? AND r.active = 1
  ...
  for (const f of due) {
    ...
    if (f.trigger === 'no_reply' && f.last_replied_at && f.parent_message_id) {
      ...
      const parent = db.prepare(`SELECT sent_at FROM ${tableForParent} WHERE id = ?`).get(f.parent_message_id);
    ...
    if (channel === 'email') {
      const t = db.prepare('SELECT * FROM email_templates WHERE id = ?').get(f.template_id);
      const vendor = db.prepare('SELECT id, email FROM vendors WHERE id = ?').get(f.vendor_id);
      ...
      const msg = db.prepare(`
        INSERT INTO messages (vendor_id, followup_id, direction, body, status)
        VALUES (?, ?, 'out', ?, 'queued')
    ...
    if (f.attempt < f.max_attempts) {
      ...
      db.prepare(`
        INSERT INTO followups (rule_id, vendor_id, parent_message_id, attempt, status, scheduled_at)
        VALUES (?, ?, ?, ?, 'pending', ?)
      `).run(f.rule_id, f.vendor_id, f.parent_message_id, f.attempt + 1, next);
    }
  }
}
...
function start() {
  ...
  setTimeout(() => { scheduleFollowups(); fireDueFollowups(); requeueScheduledMessages(); }, 10_000);
  console.log('[sched] scheduler started');
}

module.exports = { start, scheduleFollowups, fireDueFollowups, requeueScheduledMessages };
