// One-shot importer: petscare-hisar-leads.xlsx (converted to CSV) -> vendors table.
...
const csvPath = process.argv[2] || '/Users/arju/Downloads/petscare-hisar-leads.csv';
...
const tx = db.transaction((rows) => {
  ...
  for (const r of rows) {
    ...
    const allTags = ['petscare-hisar', csvTags].filter(Boolean).join(',');

    let category = null;
    const lc = csvTags.toLowerCase();
    if (lc.includes('vet')) category = 'Veterinary';
    else if (lc.includes('pet-store') || lc.includes('pet-shop')) category = 'Pet Store';
    else if (lc.includes('grooming')) category = 'Pet Grooming';
